        ...
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            if (!deviceAddresses.contains(device.getAddress())) {
                ...
                try {
                    ...
                    Log.e("Kavin-E", "---" + e.localizedMessage)
...
//            }
    ...
    //  deviceListAdapter = new ArrayAdapter<>(this, android.R.layout.simple_list_item_1, new ArrayList<>(deviceAddresses));
    ...
    private fun updateDeviceList() {
        ...
        if (deviceListAdapter == null) {
            Log.d(TAG, "DeviceListAdapter is null. Creating new adapter...")
            deviceListAdapter = ArrayAdapter(this, android.R.layout.simple_list_item_1, ArrayList(deviceAddresses))
            val deviceList = findViewById<ListView>(R.id.deviceList)
            deviceList.setAdapter(deviceListAdapter)
        } else {
            Log.d(TAG, "DeviceListAdapter already exists. Clearing and updating adapter...")
            deviceListAdapter!!.clear()
            deviceListAdapter!!.addAll(deviceAddresses)
            deviceListAdapter!!.notifyDataSetChanged()
        }
        Log.d(TAG, "DeviceListAdapter values: " + deviceListAdapter.toString())
        Log.d(TAG, "DeviceList values: $deviceAddresses")
    }
    ...
    companion object {
        ...
        private const val SMARTWATCH_MAC_ADDRESS = "F7:6B:D5:29:36:8F"
